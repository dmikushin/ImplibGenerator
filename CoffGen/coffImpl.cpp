...
template <> struct ArchTraits<ArchIA64> {
  typedef DWORD64 UIntPtr;
  enum {
...
template <class T> class CDispose : public T {
protected:
  virtual ~CDispose() = 0 {}
...
template <typename Arch> class CCoffFactory : public ICoffFactory {
public:
  ICoffBuilder *CreateCoffBuilder() { return new CCoffBuilder<Arch>(); }
  ISectionBuilder *CreateSectionBuilder() {
    return new CSectionBuilder<Arch>();
  }
  ISymbolTableBuilder *CreateSymbolTableBuilder() {
    return new CSymbolTableBuilder<Arch>();
  }
  IStringTableBuilder *CreateStringTableBuilder() {
    return new CStringTableBuilder<Arch>();
  }
  IRelocatableVar *CreateRelocatableVar() {
    return new CRelocatableVar<Arch>();
  }
  IRelocationTableBuilder *CreateRelocationTableBuilder() {
    return new CRelocationTableBuilder<Arch>();
  }
...
template <typename Arch> class CCoffBuilder : public CDispose<ICoffBuilder> {
...
public:
  ...
  ~CCoffBuilder() {
    ...
    for (i = m_sections.begin(), iend = m_sections.end(); i != iend; ++i) {
      (*i)->Dispose();
    }
  ...
  IStringTableBuilder *GetStringTableBuilder() { return m_stringTable; }

  ISymbolTableBuilder *GetSymbolTableBuilder() { return m_symbolTable; }
  ...
  void GetRawData(PBYTE p) {
    ...
    coffHeader->PointerToSymbolTable =
        0; // will assign value just before write symbol table
...
class CSectionBuilder : public CDispose<ISectionBuilder> {
  ...
  int AdjustAlign(DWORD *adjustedSize, DWORD *padSize) {
    ...
    if (m_chara & SECH_ALIGN1)
      align = 1;
    else if (m_chara & SECH_ALIGN2)
      align = 2;
    else if (m_chara & SECH_ALIGN4)
      align = 4;
    else if (m_chara & SECH_ALIGN8)
      align = 8;
    else if (m_chara & SECH_ALIGN16)
      align = 16;
    else if (m_chara & SECH_ALIGN32)
      align = 32;
    else if (m_chara & SECH_ALIGN64)
      align = 64;
    else
      align = 1;
    ...
    if (ras % align == 0)
      ...
      as = (ras / align + 1) * align;

    ps = as - ras;

    if (adjustedSize == &_outArg)
      return as;
    if (padSize == &_outArg)
      return ps;
...
public:
  ...
  ~CSectionBuilder() { m_relocTable->Dispose(); }
  ...
  void SetSectionIndex(int secIndex) { m_secIndex = secIndex; }

  int GetSectionIndex() { return m_secIndex; }

  void AppendData(LPCBYTE pBuf, int len, IRelocatableVar *aReloc[],
                  int relocCount) {
  ...
  void SetCharacteristics(DWORD p) { m_chara = p; }

  int GetHeaderLength() { return sizeof(IMAGE_SECTION_HEADER); }

  DWORD GetRawCharacteristic(DWORD c) {
    return (c & SECH_CODE ? IMAGE_SCN_CNT_CODE : 0) |
  ...
  DWORD GetRawCharacteristic() { return GetRawCharacteristic(m_chara); }
  ...
  public:
    ...
    SecAuxSym() { ZeroMemory(&u, sizeof(u)); }

    int GetDataLength() { return sizeof(u); }

    void GetRawData(PBYTE p) { *(PIMAGE_AUX_SYMBOL)p = u; }

    void Dispose() { delete this; }
  };

  ISectionAuxSymbol *CreateAuxSymbol(ISectionBuilder *associatedSection,
                                     SectionComdat selection) {
    ...
    r->u.Section.Number =
        associatedSection == 0 ? 0 : associatedSection->GetSectionIndex();
...
public:
  ...
  LPCSTR GetString(int offset) { return &m_buf[offset - sizeof(DWORD)]; }

  int GetDataLength() { return m_buf.size() + sizeof(DWORD); }
...
public:
  void SetStringTable(IStringTableBuilder *t) { m_strTable = t; }
  ...
  int AddSymbol(ISectionBuilder *section, int offset, LPCSTR name,
                StorageType stype, int auxCnt) {
    ...
    s.Type = ((stype == SYST_FUNCTION || stype == SYST_STATICFUNCTION)
                  ? (IMAGE_SYM_DTYPE_FUNCTION << 4) | 0
                  : 0);
    ...
    case SYST_FUNCTION:
      s.StorageClass = IMAGE_SYM_CLASS_EXTERNAL;
      break;
    case SYST_STATICFUNCTION:
      s.StorageClass = IMAGE_SYM_CLASS_STATIC;
      break;
    case SYST_SECTION:
      s.StorageClass = IMAGE_SYM_CLASS_SECTION;
      break;
    }
  ...
  int GetSymbolCount() { return m_buf.size(); }

  int GetDataLength() { return sizeof(IMAGE_SYMBOL) * GetSymbolCount(); }
  ...
  public:
    ...
    LPCSTR GetString(int nIndex) { return m_str[nIndex]; }
  };
...
public:
  void Offset(int offset) { m_val += offset; }

  void Set(LPCSTR symbol, ISectionBuilder *section, DWORD val, int size,
           DWORD reloctype) {
    m_symbol = symbol;
    m_val = val;
    m_size = size;
  ...
  void Get(LPCSTR *symbol, ISectionBuilder **section, DWORD *val, int *size,
           DWORD *reloctype) {
...
public:
  ~CRelocationTableBuilder() {
    ...
    for (i = m_relocs.begin(), iend = m_relocs.end(); i != iend; ++i) {
      (*i)->Dispose();
    }
  }

  int GetPtrLength() { return sizeof(ArchTraits<Arch>::UIntPtr); }

  int GetCount() { return m_relocs.size(); }

  void AppendRelocationItem(IRelocatableVar *p) { m_relocs.push_back(p); }
  ...
  void PushToSymbolTable(ISymbolTableBuilder *table) {
    ...
    for (i = m_relocs.begin(), iend = m_relocs.end(); i != iend; ++i) {
      ...
      r.SymbolTableIndex =
          table->AddSymbol(0, symbolValSize, symbolName, SYST_EXTERN, 0);

      r.Type = (relocType == VARelocate32 ? ArchTraits<Arch>::RelVA : 0) |
  ...
  int GetDataLength() { return m_buf.size() * sizeof(IMAGE_RELOCATION); }
