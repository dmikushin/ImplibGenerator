...
class IRelocationTableBuilder; // for building relocation table of coff object
                               // file
...
public:
  ...
  // call turns: add symbol -> PushRelocs. the function will reference relocs to
  // previous added symbols
...
public:
  ...
  virtual void AppendData(LPCBYTE pData, int len, IRelocatableVar *[],
                          int relocCount) = 0;
  ...
  // please don't call setsectionindex by hand unless you know what you are
  // doing. it's called by the coff builder
  ...
  virtual ISectionAuxSymbol *CreateAuxSymbol(ISectionBuilder *associatedSection,
                                             SectionComdat selection) = 0;
};
...
public:
  ...
  virtual int AddSymbol(ISectionBuilder *section, int value, LPCSTR name,
                        StorageType, int auxCnt) = 0;
...
enum RelocateType { VARelocate32 = 1, VARelocate64, RVARelocate };
...
public:
  ...
  virtual void Set(LPCSTR symbol, ISectionBuilder *section, DWORD offset,
                   int size, DWORD relocType) = 0;

  virtual void Get(LPCSTR *symbol, ISectionBuilder **section, DWORD *offset,
                   int *size, DWORD *relocType) = 0;
};
...
public:
  ...
  // generally you don't need call this method unless you know what you are
  // doing
  virtual int GetCount() = 0;
