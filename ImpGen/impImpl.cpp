...
template <class Arch> struct ArchTraits;
...
template <> struct ArchTraits<ArchX86> {
  ...
  enum { PtrReloc = VARelocate32, RvaReloc = RVARelocate };

  static ICoffFactory *GetCoffFactory() { return GetX86CoffFactory(); }
};
BYTE ArchTraits<ArchX86>::JmpMemInst[2] = {0xff, 0x25};
ArchTraits<ArchX86>::UIntPtr ArchTraits<ArchX86>::OrdinalFlag =
...
template <> struct ArchTraits<ArchX64> {
  ...
  enum { PtrReloc = VARelocate64, RvaReloc = RVARelocate };

  static ICoffFactory *GetCoffFactory() { return GetX64CoffFactory(); }
};
BYTE ArchTraits<ArchX64>::JmpMemInst[2] = {0xff, 0x25};
ArchTraits<ArchX64>::UIntPtr ArchTraits<ArchX64>::OrdinalFlag =
    IMAGE_ORDINAL_FLAG64;

template <class Arch> class CImpSectionBuilder : public IImpSectionBuilder {
  ICoffFactory *cf;

  CImpSectionBuilder() { cf = ArchTraits<Arch>::GetCoffFactory(); }
...
public:
  ICoffFactory *GetCoffFactory() { return cf; }

  int GetPtrSize() { return sizeof(ArchTraits<Arch>::UIntPtr); }
  ...
  void BuildImportDescriptor(LPCSTR szDllName, ICoffBuilder *cb) {
    ...
    relVar[0]->Set(".idata$4", idSection,
                   OFFSET(IMAGE_IMPORT_DESCRIPTOR, OriginalFirstThunk),
                   GetPtrSize(), ArchTraits<Arch>::RvaReloc);
    relVar[1]->Set(".idata$6", idSection, OFFSET(IMAGE_IMPORT_DESCRIPTOR, Name),
                   0, ArchTraits<Arch>::RvaReloc);
    relVar[2]->Set(".idata$5", idSection,
                   OFFSET(IMAGE_IMPORT_DESCRIPTOR, FirstThunk), GetPtrSize(),
                   ArchTraits<Arch>::RvaReloc);
    ...
    cb->GetSymbolTableBuilder()->AddSymbol(
        idSection, 0, BuildImpDescSymbolName(szDllName), SYST_EXTERN, 0);
    cb->GetSymbolTableBuilder()->AddSymbol(idSection, schara, ".idata$2",
                                           SYST_SECTION, 0);
    cb->GetSymbolTableBuilder()->AddSymbol(dllnameSection, 0, ".idata$6",
                                           SYST_STATIC, 0);
    cb->GetSymbolTableBuilder()->AddSymbol(0, schara, ".idata$4", SYST_SECTION,
                                           0);
    cb->GetSymbolTableBuilder()->AddSymbol(0, schara, ".idata$5", SYST_SECTION,
                                           0);
    cb->GetSymbolTableBuilder()->AddSymbol(0, 0, "__NULL_IMPORT_DESCRIPTOR",
                                           SYST_EXTERN, 0);
    cb->GetSymbolTableBuilder()->AddSymbol(0, 0, BuildNullThunkName(szDllName),
                                           SYST_EXTERN, 0);
  }

  void BuildImportThunk(LPCSTR szDllName, LPCSTR szImpName, LPCSTR szFuncName,
                        LPCSTR szDllExpName, WORD nDllExpOrdinal,
                        ICoffBuilder *cb) {
    ...
    if (szFuncName) {
      ...
      callstubSec->SetCharacteristics(
          ArchTraits<Arch>::ImpCallStubSectionChara | SECH_COMDAT);

      callstubSec->AppendData(ArchTraits<Arch>::JmpMemInst,
                              sizeof(ArchTraits<Arch>::JmpMemInst), 0, 0);

      ArchTraits<Arch>::UIntPtr zeroptr = 0;
      IRelocatableVar *funcptr = cf->CreateRelocatableVar();
      funcptr->Set(szImpName, callstubSec, 0, sizeof(zeroptr),
                   ArchTraits<Arch>::PtrReloc);
      callstubSec->AppendData((PBYTE)&zeroptr, sizeof(zeroptr), &funcptr, 1);

      cb->GetSymbolTableBuilder()->AddSymbol(callstubSec, 0, ".text",
                                             SYST_STATIC, 1);
      ISectionAuxSymbol *auxsym =
          callstubSec->CreateAuxSymbol(0, SECO_NODUPLICATE);
      cb->GetSymbolTableBuilder()->AddAuxData(auxsym);
      auxsym->Dispose();

      cb->GetSymbolTableBuilder()->AddSymbol(callstubSec, 0, szFuncName,
                                             SYST_FUNCTION, 0);
    }
    ...
    for (i = 0; i < 2; ++i) { // 0 = OriginalThunk, 1 = Thunk
      ...
      thunkSection->SetCharacteristics(ArchTraits<Arch>::ImpThunkSectionChara |
                                       SECH_COMDAT);
      ...
      if (i == 0) { // .idata$5, thunk
        cb->GetSymbolTableBuilder()->AddSymbol(thunkSection, 0, secName[i],
                                               SYST_STATIC, 1);
        ISectionAuxSymbol *secauxsym =
            thunkSection->CreateAuxSymbol(0, SECO_NODUPLICATE);
        cb->GetSymbolTableBuilder()->AddAuxData(secauxsym);
        secauxsym->Dispose();
        cb->GetSymbolTableBuilder()->AddSymbol(thunkSection, 0, szImpName,
                                               SYST_EXTERN, 0);
      } else { //.idata$4
        cb->GetSymbolTableBuilder()->AddSymbol(thunkSection, 0, secName[i],
                                               SYST_STATIC, 1);
        ISectionAuxSymbol *secauxsym =
            thunkSection->CreateAuxSymbol(id5, SECO_ASSOCIATIVE);
        cb->GetSymbolTableBuilder()->AddAuxData(secauxsym);
        secauxsym->Dispose();
      }
    }
    ...
    if (szDllExpName != 0) {
      ...
      impSection->SetCharacteristics(ArchTraits<Arch>::ImpLookupSectionChara |
                                     SECH_COMDAT);
      ...
      cb->GetSymbolTableBuilder()->AddSymbol(impSection, 0, ".idata$6",
                                             SYST_STATIC, 1);
      ISectionAuxSymbol *auxsym =
          impSection->CreateAuxSymbol(id5, SECO_ASSOCIATIVE);
      cb->GetSymbolTableBuilder()->AddAuxData(auxsym);
      auxsym->Dispose();
    }

    // cb->GetSymbolTableBuilder()->AddSymbol(0,
    // sizeof(ArchTraits<Arch>::UIntPtr), "NullThunk", SYST_EXTERN, 0);
    // cb->GetSymbolTableBuilder()->AddSymbol(0,
    // sizeof(ArchTraits<Arch>::UIntPtr), "NullOriginalThunk", SYST_EXTERN, 0);
    cb->GetSymbolTableBuilder()->AddSymbol(
        0, 0, BuildImpDescSymbolName(szDllName), SYST_EXTERN, 0);
  }

  void BuildImportByNameThunk(LPCSTR szDllName, LPCSTR szImpName,
                              LPCSTR szFuncName, LPCSTR szDllExpName,
                              ICoffBuilder *cb) {
    BuildImportThunk(szDllName, szImpName, szFuncName, szDllExpName, 0, cb);
  }

  void BuildImportByOrdinalThunk(LPCSTR szDllName, LPCSTR szImpName,
                                 LPCSTR szFuncName, WORD nDllExpOrdinal,
                                 ICoffBuilder *cb) {
  ...
  void BuildNullThunk(LPCSTR szDllName, ICoffBuilder *cb) {
    ...
    cb->GetSymbolTableBuilder()->AddSymbol(ft, 0, BuildNullThunkName(szDllName),
                                           SYST_EXTERN, 0);
    // cb->GetSymbolTableBuilder()->AddSymbol(oft, 0, "NullOriginalThunk",
    // SYST_EXTERN, 0);
  }
  ...
  void BuildNullDescriptor(ICoffBuilder *cb) {
    ...
    cb->GetSymbolTableBuilder()->AddSymbol(
        nullDesc, 0, "__NULL_IMPORT_DESCRIPTOR", SYST_EXTERN, 0);
  }
...
IImpSectionBuilder *GetX86ImpSectionBuilder() {
  return &CImpSectionBuilder<ArchX86>::Instance;
}
IImpSectionBuilder *GetX64ImpSectionBuilder() {
  return &CImpSectionBuilder<ArchX64>::Instance;
}
}
}; // namespace Sora
