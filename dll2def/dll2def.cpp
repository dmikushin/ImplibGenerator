...
int cprintf(char* dest, const char* mask, int n) {
    int len = n <= 9 ? 1 : n <= 99 ? 2 : n <= 999 ? 3 : n <= 9999 ? 4 : n <= 99999 ? 5 : 6;
    char* outp = dest;
    char c;
    while ((c = *mask++)) {
        if (c == '%') {
...
int parse_pe(const std::string& filename, std::ifstream& hFile, std::ofstream& hOut, bool compact) {
    ...
    uint32_t dll_name_len, aux, i, file_pos, size_of_optional_header, is_PE32plus, num_sections;
    uint32_t ordinal_base, num_pointers, ordinals_array, pointers_array, psymbols_array, pub_name_len;
    uint16_t ordinal;

    // Open the file
    hFile.open(filename, std::ios::binary);
    if (!hFile.is_open()) return ERR_FILE_NOT_FOUND;

    // Read in a fragment of the MSDOS header
    hFile.read(buf, 0x40);
    if (hFile.gcount() != 0x40) return ERR_BAD_FORMAT;
    ...
    hFile.read(buf, 0x1A);
    if (hFile.gcount() != 0x1A || *reinterpret_cast<uint32_t*>(buf) != 0x4550) return ERR_BAD_FORMAT;
    ...
    if (hFile.gcount() != 8) return ERR_BAD_FORMAT;
    ...
    while (num_sections--) {
        hFile.read(buf, 0x28);
        if (hFile.gcount() != 0x28) return ERR_BAD_FORMAT;
        *current_node = new RVA_2_FileOffset_node;
        if (!*current_node) return ERR_NO_FREE_MEM;
        (*current_node)->node.VirtualAddress = *reinterpret_cast<uint32_t*>(buf + 0x0C);
        (*current_node)->node.VirtualSize = *reinterpret_cast<uint32_t*>(buf + 0x08);
        (*current_node)->node.FileOffset = *reinterpret_cast<uint32_t*>(buf + 0x14);
    ...
    if (!file_pos) return ERR_BAD_FORMAT;
    hFile.seekg(file_pos, std::ios::beg);
    ...
    pointers_array = do_RVA_2_FileOffset(sections, *reinterpret_cast<uint32_t*>(buf + 0x1C));
    psymbols_array = do_RVA_2_FileOffset(sections, *reinterpret_cast<uint32_t*>(buf + 0x20));
    ordinals_array = do_RVA_2_FileOffset(sections, *reinterpret_cast<uint32_t*>(buf + 0x24));
    if (!num_pointers || !num_sections || !pointers_array || !psymbols_array || !ordinals_array) return ERR_NO_EXPORT;
    ...
    while (num_sections--) {
        ...
        if (!pub_name_len) pub_name_len = cprintf(pub_name, "ord.%", ordinal + ordinal_base);
        if (!compact) {
            // ; DLLNAME.NAME ord.#
            buf[0] = ';';
            buf[1] = ' ';
